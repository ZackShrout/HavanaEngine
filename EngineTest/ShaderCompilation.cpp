...
namespace
{
	...
	public:
		...
		dxc_compiled_shader compile(shader_file_info info, std::filesystem::path full_path, havana::utl::vector<std::wstring>& extra_args)
		{
		...
		dxc_compiled_shader compile(IDxcBlobEncoding* sourceBlob, havana::utl::vector<std::wstring> compiler_args)
		{
			DxcBuffer buffer{};
	...
	private:
		utl::vector<std::wstring> get_args(const shader_file_info& info, utl::vector<std::wstring>& extra_args)
		{
			...
			args.emplace_back(L"-Qstrip_debug");								// strip debug info into a separate blob
